...
def main() -> None:
    ...
    # FIX: increased timeouts + retry_on_error for stable polling on Railway
    ...
    # FIX: drop_pending_updates=True prevents update flood after restart
    # FIX: retry_on_error=True makes the bot auto-reconnect on network issues
    app.run_polling(
        allowed_updates=Update.ALL_TYPES,
        drop_pending_updates=True,
        retry_after_network_errors=True,
    )


if __name__ == "__main__":
    # FIX: Outer retry loop — if the bot crashes for any reason,
    # wait 5 seconds and restart automatically instead of dying
    while True:
        try:
            main()
            ...
            logger.error("Bot crashed with error: %s — restarting in 5 seconds...", e)
            time.sleep(5)
